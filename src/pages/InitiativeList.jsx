...
const listSchema = Yup.object({
  initiatives: Yup.array().of(
    Yup.object().shape({
      ...
      initiative: Yup.number().default(0).required('Digite a iniciativa do personagem'),
    }),
  ),
});
  ...
  return (
    <>
      ...
      {fieldsInitiatives.length > 0 ? (
        <Container component="form" sx={{ mt: 1, p: 2, borderRadius: 2, bgcolor: 'background.paper' }} maxWidth="md">
          <Grid container spacing={1}>
            <Grid item xs={12} sm={12} md={9} mb={1}>
              <Button text="PRoximo" onClick={pushOrder} />
            </Grid>

            <Grid item xs={12} sm={12} md={3} mb={1}>
              <Button text="limpar" color="error" onClick={() => resetField('initiatives')} />
            </Grid>
          </Grid>
        </Container>
      ) : null}
      ...
      {fieldsInitiatives.map((field, index) => (
        ...
        >
          <Grid container spacing={1}>
            ...
            {order === index ? (
              <Grid item xs={12} sm={12} md={1} textAlign="center" alignSelf="center">
                <CenterFocusStrongIcon sx={{ color: 'primary.contrastText' }} fontSize="large" />
              </Grid>
            ) : null}
          </Grid>
        </Container>
